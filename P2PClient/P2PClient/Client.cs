...
namespace P2PChat
{
    public class Client
    {
        public static IPEndPoint ServerEndpoint = new IPEndPoint(IPAddress.Parse("192.168.10.3"), 50);
      ...
      {
        FileMessage m = (FileMessage)Item;
        ClientInfo CI = Clients.FirstOrDefault(x => x.ID == Item.ID);

        if (m.ID == 0)
          if (OnResultsUpdate != null)
            OnResultsUpdate.Invoke(this, m.From + ": " + m.FileName);
    ...
    public MessageReceivedEventArgs(ClientInfo _clientInfo, FileMessage _fileMessage, IPEndPoint _establishedEP)
    {
      clientInfo = _clientInfo;
